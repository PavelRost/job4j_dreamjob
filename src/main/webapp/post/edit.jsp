...
<head>
    ...
    <script>
        function validate() {
            var rsl = true;
            var valuePost = $('#editPost').val();
            if (valuePost.length === 0) {
                alert($('#editPost').attr('title'));
                rsl = false;
            }
            return rsl;
        }
            ...
            <div class="card-body">
                <form action="<%=request.getContextPath()%>/posts.do?id=<%=post.getId()%>" method="post">
                    <div class="form-group">
                        <label>Имя</label>
                        <input type="text" class="form-control" title="Введите новый заголовок для вакансии" id="editPost" name="name" value="<%=post.getName()%>">
                    </div>
